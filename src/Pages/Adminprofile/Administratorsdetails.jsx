  ...
  const fetchOrderById = async (adminId) => {
    try {
      ...
      const response = await axios.get(
        `${BACKEND_URL}/admin/AdminprofileDetailsById/${admin_Id}`
      );
          ...
          <Box sx={{ mx: 4 }}>
            <Typography sx={{ fontSize: 30, fontWeight: "bold" }}>
              {Administratordetails ? (
                <div>
                  <AppsIcon sx={{ mr: 3 }} />
                  AdminID - {Administratordetails.admin_Id}
                </div>
              ) : (
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={4}>
                      <Box sx={{ p: 1, border: "1px solid grey" }}>
                        {Administratordetails ? (
                          <div>
                            Name :{Administratordetails.FirstName}
                          </div>
                        ) : (
                          <div>Loading order details...</div>
                        )}
                    ...
                    <Grid item xs={12} md={4}>
                      <Box sx={{ p: 1, border: "1px solid grey" }}>
                        {Administratordetails ? (
                          <div>
                             Last Name :{Administratordetails.LastName}
                          </div>
                        ) : (
                          <div>Loading order details...</div>
                        )}
                    ...
                    <Grid item xs={12} md={4}>
                      <Box sx={{ p: 1, border: "1px solid grey" }}>
                        {Administratordetails ? (
                          <div>
                            Telephone :{Administratordetails.Tele}
                          </div>
                        ) : (
                          <div>Loading order details...</div>
                        )}
                    ...
                    <Grid item xs={12} md={3}>
                      <Box sx={{ p: 1, border: "1px solid grey" }}>
                        {Administratordetails ? (
                          <div>
                             Street_No :{Administratordetails.Street_No}
                          </div>
                        ) : (
                          <div>Loading order details...</div>
                        )}
                    ...
                    <Grid item xs={12} md={3}>
                      <Box sx={{ p: 1, border: "1px solid grey" }}>
                        {Administratordetails ? (
                          <div>
                             Street :{Administratordetails.Street}
                          </div>
                        ) : (
                          <div>Loading order details...</div>
                        )}
                    ...
                    <Grid item xs={12} md={4}>
                      <Box sx={{ p: 1, border: "1px solid grey" }}>
                        {Administratordetails ? (
                          <div>
                              City :{Administratordetails. City}
                          </div>
                        ) : (
                          <div>Loading order details...</div>
                        )}
